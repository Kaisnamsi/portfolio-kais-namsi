import React from 'react';
import heroImage from '../assets/heroImage.jpg';
import { MdKeyboardArrowRight } from 'react-icons/md';
import { Link, animateScroll as scroll } from 'react-scroll'; // Import Link and scroll

const Home = () => {
  return (
    <div name="home" className="h-screen bg-gradient-to-b from-black to-gray-800">
      <div className="max-w-screen-lg mx-auto flex flex-col items-center justify-center h-full px-4 md:flex-row">
        <div className="flex flex-col justify-center h-full">
          <h2 className="text-4xl sm:text-7xl font-bold text-white">I'm a Full Stack Developer</h2>
          <p className="text-gray-500 py-4 max-x-md">
            I'm Kais, a passionate Fullstack Web Developer.
            With expertise in crafting dynamic user interfaces using tools like React, Redux, and Next.js,
            also, I manage the backend operations to ensure everything runs smoothly. Let's collaborate and create remarkable digital experiences together.
          </p>
          <div>
            <Link
              to="portfolio"
              spy={true}
              smooth={true}
              offset={-70} 
              duration={1000}
            >
              <button className="group text-white w-fit px-6 py-3 my-2 flex item-center rounded-md bg-gradient-to-r from-cyan-500 to-blue-500 cursor-pointer">
                Portfolio
                <span className="group-hover:rotate-90 duration-300">
                  <MdKeyboardArrowRight size={25} className="ml-1" />
                </span>
              </button>
            </Link>
          </div>
        </div>
        <div>
          <img src={heroImage} alt="my profile" className="rounded-2xl mx-auto w-2/3 md:w-full" />
        </div>
      </div>
    </div>
  );
}

export default Home;
